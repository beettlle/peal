...
| **Parallel execution** | Optional. Tasks marked parallelizable in the plan (per §4) SHALL be run in parallel within a block; consecutive marked tasks form one block. Phases 1 and 2 run concurrently per task; phase 3 (stet) SHALL run sequentially per task after all phase 2s in the block complete (per §5). |
...
- **Format:** Tasks are second-level headings: `## Task 1`, `## Task 2`, … (digit sequence), optionally with a `(parallel)` suffix (see Parallel blocks below). Task content is everything from the end of that heading line until the next line that matches a task heading or EOF.
- **Parsing rule:** A task heading SHALL match the pattern `^## Task\s+(\d+)\s*(\(parallel\))?\s*$` (after stripping CRLF to LF). The first capturing group is the task index; the second, if present, indicates the task is parallelizable. Task body = lines after the heading until the next line matching `^## Task\s+\d+` (with optional `\s*(\(parallel\))?\s*`) or end of file. Tasks SHALL be ordered by ascending index (1, 2, 3, …). Gaps (e.g. Task 1, Task 3) are allowed but the orchestrator SHALL process in numerical order.
...
Tasks MAY be marked as parallelizable so that the orchestrator runs them concurrently for a performance boost.

- **Heading pattern with parallel marker:** A task heading MAY include an optional suffix `(parallel)`, e.g. `## Task 2 (parallel)`, `## Task 3 (parallel)`. The full pattern SHALL be `^## Task\s+(\d+)\s*(\(parallel\))?\s*$` (after stripping CRLF to LF). The first capturing group is the task index; the second indicates whether the task is parallelizable.
- **Parallel block:** Consecutive tasks that share the `(parallel)` marker form one **parallel block**. The orchestrator SHALL run all tasks in that block concurrently. Within each task's execution, phase 1 → 2 → 3 still run in order; there is no shared conversation across tasks. Tasks without the marker, or a run of only one task (even if marked parallel), run sequentially.
- **Backward compatibility:** Plans with no `(parallel)` marker behave exactly as today (strictly sequential). No change to task body content or encoding.
...
When the plan contains one or more parallel blocks (§4), the orchestrator SHALL execute tasks as follows.

- **Sequential segments:** Tasks that are not in a parallel block run one after another (current behavior).
- **Parallel block:** For a set of tasks in one parallel block, the orchestrator SHALL run one execution stream per task (e.g. one process or async task per task index). Each stream runs that task's phase 1, then phase 2, **concurrently** with other tasks in the block. The orchestrator waits for all phase 2s in the block to complete before starting phase 3 for any task in the block.
- **Phase 3 (stet) within a parallel block:** To avoid conflicting stet invocations in the same repo, phase 3 (stet and address) SHALL NOT run concurrently across tasks in a block. After all phase 2s in the block are done, the orchestrator SHALL run phase 3 **sequentially** for each task in the block (one task at a time). So parallelism applies to phases 1 and 2 only; phase 3 runs in task order after the block's phase 2s complete.
...
**Execution order with parallel blocks:** When the plan contains parallel blocks (§4), execution flows sequentially by block; within a parallel block, tasks run concurrently (phases 1 and 2), then phase 3 runs sequentially per task.

```mermaid
flowchart LR
  subgraph seq1 [Sequential]
    T1[Task A 1-2-3]
  end
...
User provides path to feature plan markdown and path to target repo (via config or CLI). Orchestrator parses tasks (§4), then for each task runs phases 1 → 2 → 3 (with 3b–3c loop as in §5). When the plan contains parallel blocks (§4), the orchestrator runs tasks within each block concurrently (phases 1 and 2), then phase 3 sequentially per task in the block, then proceeds to the next block or task. After all tasks: optional "final stet review" and "final brutal audit" (orchestrator runs user-configured commands, captures output; no Cursor call). See FR-9.
...
On startup with "resume," orchestrator SHALL load state, skip tasks whose index is in `completed_task_indices`, and start from the smallest task index not in that set. If the next work to run is part of a parallel block, the orchestrator SHALL run all tasks in that block that are not in `completed_task_indices` (in parallel for phases 1 and 2, then phase 3 sequentially); completed tasks in the block SHALL be skipped. If state file is missing or corrupted, orchestrator SHALL treat as "no resume": start from task 1 and SHALL warn the user (e.g. to stderr).
...
| FR-8 | Configurable: agent command name, repo path, plan path, stet command sequence, sandbox, model override, timeouts, max address rounds, parallel execution (e.g. `parallel: true/false` or `max_parallel: N` for debugging or resource limits). |
...
| FR-10 | Parse parallel marker from the plan per §4 and identify parallel blocks. Run tasks in parallel when they belong to the same block: run phases 1 and 2 concurrently per task; when all phase 2s in the block are done, run phase 3 sequentially for each task in the block. |
...
- **Configuration:** File (e.g. TOML or YAML) and/or environment variables and/or CLI flags. Precedence SHALL be defined (e.g. CLI &gt; env &gt; config file). Config keys SHALL include at least: `agent_cmd`, `plan_path`, `repo_path`, `stet_commands` (or equivalent), `sandbox`, `model`, `max_address_rounds`, `state_dir`, `phase_timeout_sec` (or equivalent), and optionally `parallel` (enable/disable) and `max_parallel` (cap on concurrent Cursor CLI processes).
- **Concurrency:** When parallel blocks are used, the orchestrator SHALL use safe concurrency (e.g. Rust threads or async tasks); subprocess invocations SHALL NOT share mutable state. Config MAY include `max_parallel` to cap concurrent Cursor CLI processes (e.g. to avoid resource exhaustion).
...
| **One task in a parallel block fails** | Persist state; add only completed task indices for that block to `completed_task_indices`. Fail the run (exit non-zero) unless config says "continue with remaining tasks" (then complete other tasks in the block and proceed). |
| **Parallel block with single task** | Treat as sequential (no concurrency). |
| **Parallel disabled via config** | Ignore parallel marker; run all tasks sequentially. |
